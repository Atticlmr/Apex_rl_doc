...
MAPPO 和 IPPO 共用多智能体 runner 与 rollout storage。MAPPO 使用 CTDE
结构：actor 输入每个 agent 的局部观测，critic 可以输入环境提供的全局
state。IPPO 保持相同的 per-agent actor 接口，但通过
``centralized_critic=False`` 让每个 critic 只输入对应 agent 的局部观测。
